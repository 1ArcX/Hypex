...
function ExpenseModal({ onClose, onSave, editing, categories = CATEGORIES, defaultDate, plannedMode }) {
  ...
  return ReactDOM.createPortal(
    <div ref={backdropRef} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 'var(--keyboard-height, 0px)', background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(10px)', zIndex: 9999, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      <div style={{ width: '100%', maxWidth: 480, background: 'var(--bg-sidebar)', borderRadius: '22px 22px 0 0', border: '1px solid var(--border)', borderBottom: 'none', padding: '20px 20px calc(24px + env(safe-area-inset-bottom))', maxHeight: '90%', overflowY: 'auto', WebkitOverflowScrolling: 'touch', animation: 'sheetUp 0.3s cubic-bezier(0.34,1.1,0.64,1)' }}>
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg-base)' }}>
                ...
                {recurringIncome.map(src => {
                  ...
                  return (
                    <div key={src.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      {isCurrentMonth && (
                        <button
                          onClick={() => setShowEarlyIncome(src)}
                          style={{ padding: '5px 9px', borderRadius: 8, background: isToday ? '#10B981' : 'rgba(16,185,129,0.12)', border: `1px solid ${isToday ? 'transparent' : 'rgba(16,185,129,0.35)'}`, color: isToday ? '#000' : '#10B981', cursor: 'pointer', fontSize: 11, fontWeight: 600, whiteSpace: 'nowrap' }}>
      ...
      {showEarlyIncome && (
        <IncomeDayModal
          ...
          onDone={() => { setShowEarlyIncome(null); fetchAll() }}
        />
      )}
